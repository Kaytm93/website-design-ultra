 ...
 * motion holds the product's seeded static pose — the strongest static shot,
 * per 3d-art-direction — while full motion rotates it from the injected
 * clock. The choice is persisted, so an explicit user setting is preserved
 * across sessions. In deterministic mode the control is locked:
 * WDU_REDUCED_MOTION is the capture contract and input must not move it.
 */
    ...
    <fieldset className="motion-control" disabled={locked}>
      ...
      <p className="motion-note">
        {locked
          ...
          : motion === 'reduced'
            ? 'Reduced motion: the product holds its seeded static pose; copy, controls, and quality stay available.'
            : 'Full motion: the product rotates from the injected clock.'}
      </p>
    </fieldset>
  )
}
