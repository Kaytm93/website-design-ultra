...
export function SceneCanvas({
  ...
  return (
    <Canvas
      ...
      camera={{ fov: 35, near: 0.1, far: 100, position: [0, 0.9, 4.2] }}
      className="scene-canvas"
      aria-hidden="true"
    >
      <ambientLight intensity={0.55} />
      <directionalLight position={[3, 4, 2]} intensity={2.4} />
      <directionalLight position={[-4, 1, 3]} intensity={0.5} color="#aebfd8" />
      <pointLight position={[-3, 1, -2]} intensity={60} color="#ffb86b" />
      <SceneRuntime
        mode={mode}
        stationId={stationId}
        motion={motion}
