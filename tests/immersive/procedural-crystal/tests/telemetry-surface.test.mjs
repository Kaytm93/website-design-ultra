...
test('deterministic mode reports the declared fixed-step window and validates', () => {
  ...
  assert.equal(document.runtime.frame.warmGpu.samples.length, 12)
...
test('live mode computes median and p95 from the recorded deltas', async () => {
  ...
  // Live mode: collect keeps whatever was recorded.
  assert.ok(samples.length >= 180, `expected at least 180 samples, got ${samples.length}`)
})
