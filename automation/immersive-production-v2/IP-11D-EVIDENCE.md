...
| Two generated immersive fixtures pass install, build, runtime, keyboard, mobile, reduced-motion and fallback checks. | Both fixtures exist (`tests/immersive/product-hero/`, `tests/immersive/procedural-crystal/`) and their offline suites pass on a clean checkout — 36/36 and 32/32. `tests/immersive/evaluation/run-implementation-evaluation.mjs` exercises install / build / runtime / keyboard / mobile / reduced-motion / fallback for both, and the `immersive-evaluation` CI job wires it without silent skips. **The live run has no green result at the 2.0.0 tip:** with the second fixture added it exceeds the job's own `timeout-minutes: 45` and is cancelled (`main` at `e5a1ea0` and `a9b432c`, runs 33320715644 and 33326025652). The last green `immersive-evaluation` is `3214f12` (run 33031127441), which predates `procedural-crystal`. An offline suite is not the live gate, and a cancelled run is not a pass. | **UNVERIFIED at 2.0.0** — the evaluation must fit its CI budget before this line can read PASS |
