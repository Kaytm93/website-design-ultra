...
|---|---|---|
| Two runs of one fixture produce byte-identical captures. | `volume_research/scripts/reproduce.sh` runs `volume_research/benchmark.py` twice against `volume_research/source-representation.json` (declared id `crystal-fog-density-32`). The script reports `REPRODUCIBILITY PASS` for the structured fields and `BYTE-LEVEL REPRODUCIBILITY PASS` for the encoded artefacts (slice PNGs, `packed.wduv`, `points.glb`). Hash diff is empty. | PASS |
...
| Two generated immersive fixtures pass install, build, runtime, keyboard, mobile, reduced-motion and fallback checks. | `tests/immersive/procedural-crystal/` is the second generated fixture (alongside `tests/immersive/product-hero/`). `tests/immersive/evaluation/run-implementation-evaluation.mjs` exercises install / build / runtime / keyboard / mobile / reduced-motion / fallback for both. The deterministic comparator in `volume_research/scripts/reproduce.sh` proves byte-identical capture. | PASS |
...
Validation passed: ... manifest version 1.9.1
```
...
1. Cut `1.10` only after PR 4 is merged and every `T0.1`–`T0.3`
   ...
   evidenced by the byte-identical capture in
   `volume_research/scripts/reproduce.sh`. T0.2 (frame telemetry) is
...
given. The committed binary deltas (procedural-crystal glbs, slice PNGs,
packed volume blob) reproduce byte-identically across two clean
`volume_research/benchmark.py --out` runs.
